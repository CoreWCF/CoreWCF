...
{
    public class SamlAudienceValidationTests
    {
        private const string TestIssuer = "urn:test:issuer";
        private const string TestSubjectName = "user@example.com";

        [Fact]
        public void TestSamlTokenValidationWithoutAudienceRestriction()
        {
            using X509Certificate2 certificate = CreateTestCertificate();
            string assertionXml = CreateSignedAssertionWithoutAudience(certificate);
            CoreWCF.IdentityModel.Tokens.SamlSecurityTokenHandler handler = CreateHandlerThatSkipsAudienceValidation(certificate);
        ...
        private static string CreateSignedAssertionWithoutAudience(X509Certificate2 certificate)
        {
            ...
            MSamlTokens.SamlConditions conditions = new MSamlTokens.SamlConditions(
                now.AddMinutes(-1),
                now.AddHours(1),
                Array.Empty<MSamlTokens.SamlCondition>());
        ...
        private static CoreWCF.IdentityModel.Tokens.SamlSecurityTokenHandler CreateHandlerThatSkipsAudienceValidation(X509Certificate2 certificate)
        {
        ...
        private static X509Certificate2 CreateTestCertificate()
        {
            ...
            CertificateRequest request = new CertificateRequest(
                "CN=SamlAudienceValidationTests",
                rsa,
