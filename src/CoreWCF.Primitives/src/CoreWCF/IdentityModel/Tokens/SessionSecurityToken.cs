...
using SysClaimTypes = CoreWCF.IdentityModel.Claims.ClaimTypes;
...
namespace CoreWCF.IdentityModel.Tokens
{
    ...
    public class SessionSecurityToken : SecurityToken, ISerializable
    {
        ...
        private static readonly Dictionary<string, string> DomainNameMap = new Dictionary<string, string>(MaxDomainNameMapSize, StringComparer.OrdinalIgnoreCase);
        private const int MaxDomainNameMapSize = 50;
        private readonly string _context;
        private bool _isPersistent;
        private readonly ClaimsPrincipal _claimsPrincipal;
        private readonly SctAuthorizationPolicy _sctAuthorizationPolicy;
        private readonly string _endpointId;
        private bool _isReferenceMode;
        private readonly bool _isSecurityContextSecurityTokenWrapper;
        private readonly string _id;
        private readonly SysUniqueId _contextId;
        private readonly SysUniqueId _keyGeneration;
        private readonly DateTime _keyEffectiveTime;
        private readonly DateTime _keyExpirationTime;
        private readonly Uri _secureConversationVersion;
        ...
        public SessionSecurityToken(ClaimsPrincipal claimsPrincipal,
                                     ...
                                     DateTime? validTo,
                                     SymmetricSecurityKey key)
            : this(claimsPrincipal, contextId, SecurityUniqueId.Create().Value, context, key?.GetSymmetricKey(), endpointId, validFrom, validTo, null, validFrom, validTo, null, null)
        {
        }
            ...
            : base()
        {
            ...
            {
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperArgumentNull(nameof(claimsPrincipal));
            }
            ...
            {
                validFromEffective = validFrom.Value.ToUniversalTime();
            }
            else
            {
            ...
            if (validTo.HasValue)
            {
                validToEffective = validTo.Value.ToUniversalTime();
            }
            else
            {
                validToEffective = validFromEffective.Add(SessionSecurityTokenHandler.DefaultTokenLifetime);
            }

            if (validFromEffective >= validToEffective)
            {
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new ArgumentOutOfRangeException(nameof(validFrom)));
            }

            if (validToEffective < DateTime.UtcNow)
            {
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new ArgumentOutOfRangeException(nameof(validTo)));
            }
            ...
            {
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new ArgumentOutOfRangeException(nameof(keyEffectiveTime)));
            }

            if (keyExpirationTime.Value > validToEffective)
            {
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new ArgumentOutOfRangeException(nameof(keyExpirationTime)));
            }
            ...
            if (key == null)
            {
                ...
                key = CryptoHelper.GenerateSymmetricKey(128);
            }
        ...
        protected SessionSecurityToken(SerializationInfo info, StreamingContext context)
        {
            ...
            {
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperInvalidOperation(SR.Format(SR.ID4272));
            }
            ...
            using (XmlDictionaryReader reader = XmlDictionaryReader.CreateBinaryReader(cookie, 0, cookie.Length, dictionary, XmlDictionaryReaderQuotas.Max, null, null))
            {
                ...
                {
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new SecurityTokenException(SR.Format(SR.ID4230, dictionary.SecurityContextToken.Value, reader.Name)));
                }

                string version = reader.ReadElementString();
                if (version != SupportedVersion)
                {
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new SecurityTokenException(SR.Format(SR.ID4232, version, SupportedVersion)));
                }
                ...
                {
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(SR.Format(SR.ID4239, dictionary.Id.Value)));
                }

                if (!reader.IsStartElement(dictionary.ContextId, dictionary.EmptyString))
                {
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new SecurityTokenException(SR.Format(SR.ID4230, dictionary.ContextId.Value, reader.Name)));
                }
                ...
                {
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new SecurityTokenException(SR.Format(SR.ID4230, dictionary.Key.Value, reader.Name)));
                }
                ...
                {
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new SecurityTokenException(SR.Format(SR.ID4230, dictionary.EffectiveTime.Value, reader.Name)));
                }
                ...
                {
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new SecurityTokenException(SR.Format(SR.ID4230, dictionary.ExpiryTime.Value, reader.Name)));
                }
                ...
                {
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new SecurityTokenException(SR.Format(SR.ID4230, dictionary.KeyEffectiveTime.Value, reader.Name)));
                }
                ...
                {
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new SecurityTokenException(SR.Format(SR.ID4230, dictionary.KeyExpiryTime.Value, reader.Name)));
                }
        ...
        /// </summary>
        public SysUniqueId ContextId => _contextId;
        ...
        /// </summary>
        internal bool IsSecurityContextSecurityTokenWrapper => _isSecurityContextSecurityTokenWrapper;

        /// <summary>
        /// The effective date/time of the key in this token
        /// </summary>
        public DateTime KeyEffectiveTime => _keyEffectiveTime;

        /// <summary>
        /// The expiration date/time of the key in this token
        /// </summary>
        public DateTime KeyExpirationTime => _keyExpirationTime;

        /// <summary>
        /// The identifier for the key generation in this token
        /// </summary>
        public SysUniqueId KeyGeneration => _keyGeneration;
        ...
        /// token.
        /// </summary>
        internal SctAuthorizationPolicy SctAuthorizationPolicy => _sctAuthorizationPolicy;

        /// <summary>
        /// Gets the SecureConversationVersion used for this token.
        /// </summary>
        public Uri SecureConversationVersion => _secureConversationVersion;
        ...
        public virtual void GetObjectData(SerializationInfo info, StreamingContext context)
        {
            ...
            using (XmlDictionaryWriter dicWriter = XmlDictionaryWriter.CreateBinaryWriter(stream, dictionary))
            {
                ...
                if (IsSecurityContextSecurityTokenWrapper)
                {
                ...
                // SecureConversation Version
                //
                dicWriter.WriteElementString(dictionary.SecureConversationVersion, dictionary.EmptyString, SecureConversationVersion.AbsoluteUri);

                //
                // ID and ContextId
                //
                dicWriter.WriteElementString(dictionary.Id, dictionary.EmptyString, Id);
                XmlUtil.WriteElementStringAsUniqueId(dicWriter, dictionary.ContextId, dictionary.EmptyString, ContextId.ToString());

                //
                // Key material
                //
                byte[] key = ((SymmetricSecurityKey)SecurityKeys[0]).GetSymmetricKey();
                ...
                {
                    XmlUtil.WriteElementStringAsUniqueId(dicWriter, dictionary.KeyGeneration, dictionary.EmptyString, KeyGeneration.ToString());
                }

                //
                // Effective and Expiry dates
                //
                XmlUtil.WriteElementContentAsInt64(dicWriter, dictionary.EffectiveTime, dictionary.EmptyString, ValidFrom.ToUniversalTime().Ticks);
                XmlUtil.WriteElementContentAsInt64(dicWriter, dictionary.ExpiryTime, dictionary.EmptyString, ValidTo.ToUniversalTime().Ticks);
                XmlUtil.WriteElementContentAsInt64(dicWriter, dictionary.KeyEffectiveTime, dictionary.EmptyString, KeyEffectiveTime.ToUniversalTime().Ticks);
                XmlUtil.WriteElementContentAsInt64(dicWriter, dictionary.KeyExpiryTime, dictionary.EmptyString, KeyExpirationTime.ToUniversalTime().Ticks);
                ...
                if (SctAuthorizationPolicy != null)
                {
                    ...
                    SysClaim identityClaim = ((CoreWCF.IdentityModel.Claims.DefaultClaimSet)((IAuthorizationPolicy)SctAuthorizationPolicy).Issuer)[0];
                    SerializeSysClaim(identityClaim, dicWriter);
                    dicWriter.WriteEndElement();
                }

                dicWriter.WriteElementString(dictionary.EndpointId, dictionary.EmptyString, EndpointId);
                dicWriter.WriteEndElement();
                dicWriter.Flush();
        ...
        /// </summary>
        /// <param name=nameof(dictionaryReader)>XmlDictionaryReader positioned at dictionary.ClaimsPrincipal.</param>
        ...
        private ClaimsPrincipal ReadPrincipal(XmlDictionaryReader dictionaryReader, SessionDictionary dictionary)
        {
            ...
            {
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperArgumentNull(nameof(dictionaryReader));
            }

            if (dictionary == null)
            {
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperArgumentNull(nameof(dictionary));
            }
        ...
        /// </summary>
        /// <param name=nameof(dictionaryReader)>XmlDictionaryReader positioned at dictionary.Identities</param>
        /// <param name=nameof(dictionary)>SessionDictionary to provide dictionary strings.</param>
        /// <param name=nameof(identities)>A collection of <see cref="ClaimsIdentity"/> to populate.</param>
        ...
        /// </summary>
        /// <param name=nameof(dictionaryReader)>XmlDictionaryReader positioned at dictionary.Identity.</param>
        /// <param name=nameof(dictionary)>SessionDictionary to provide dictionary strings.</param>
        /// <exception cref="ArgumentNullException">The input argument 'dictionaryReader' or 'dictionary' is null.</exception>
        ...
        private ClaimsIdentity ReadIdentity(XmlDictionaryReader dictionaryReader, SessionDictionary dictionary)
        {
            ...
            {
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new SecurityTokenException(SR.Format(SR.ID3007, dictionaryReader.LocalName, dictionaryReader.NamespaceURI)));
            }
...
#pragma warning restore SYSLIB0011
        ...
        private string GetUpn(string windowsLogonName)
        {
            ...
            {
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperArgumentNull(nameof(windowsLogonName));
            }
            ...
            if ((delimiterPos < 0) || (delimiterPos == 0) || (delimiterPos == windowsLogonName.Length - 1))
            {
                ...
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(SR.Format(SR.ID4248, windowsLogonName)));
            }
        ...
        /// </summary>
        /// <param name=nameof(dictionaryReader)>XmlDictionaryReader positioned at dictionary.Claim.</param>
        /// <param name=nameof(dictionary)>SessionDictionary to provide dictionary strings.</param>
        ...
        private void ReadClaims(XmlDictionaryReader dictionaryReader, SessionDictionary dictionary, Collection<Claim> claims)
        {
            if (dictionaryReader == null)
            {
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperArgumentNull(nameof(dictionaryReader));
        ...
        /// </summary>
        /// <param name=nameof(dictionaryReader)>XmlDictionaryReader positioned at the element dictionary.ClaimProperties </param>
        /// <param name=nameof(dictionary)>SessionDictionary to provide dictionary strings.</param>
        /// <param name=nameof(properties)>Dictionary to add properties to.</param>
        ...
        private void ReadClaimProperties(XmlDictionaryReader dictionaryReader, SessionDictionary dictionary, IDictionary<string, string> properties)
        {
            ...
            while (dictionaryReader.IsStartElement(dictionary.ClaimProperty, dictionary.EmptyString))
            {
                ...
                {
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new SecurityTokenException(SR.Format(SR.ID4249)));
                }

                if (string.IsNullOrEmpty(value))
                {
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new SecurityTokenException(SR.Format(SR.ID4250)));
                }
        ...
        private void WritePrincipal(XmlDictionaryWriter dictionaryWriter, SessionDictionary dictionary, ClaimsPrincipal principal)
        {
            ...
            {
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperArgumentNull(nameof(dictionaryWriter));
            }
            ...
            if (principal == null)
            {
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperArgumentNull(nameof(principal));
            }
        ...
        /// <param name="dictionaryWriter">XmlDictionaryWriter to write to.</param>
        /// <param name=nameof(dictionary)>SessionDictionary to provide dictionary strings.</param>
        /// <param name=nameof(identities)>The collection of ClaimsIdentity to write.</param>
...
#pragma warning restore SYSLIB0011
        ...
        /// <returns></returns>
        private delegate bool OutboundClaimsFilter(Claim claim);
        ...
        private void WriteClaims(XmlDictionaryWriter dictionaryWriter, SessionDictionary dictionary, IEnumerable<Claim> claims, OutboundClaimsFilter outboundClaimsFilter)
        {
            if (dictionaryWriter == null)
            {
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperArgumentNull(nameof(dictionaryWriter));
        ...
        /// <param name="dictionaryWriter">XmlDictionaryWriter to write to.</param>
        /// <param name=nameof(dictionary)>SessionDictionary to provide dictionary strings.</param>
        /// <param name=nameof(properties)>ClaimProperties to write.</param>
        ...
        /// <summary>
        /// Serializes the given <see cref="CoreWCF.IdentityModel.Claims.Claim"/> to the given XmlDictionaryWriter.
        /// </summary>
        ...
        private void SerializeSysClaim(SysClaim claim, XmlDictionaryWriter writer)
        {
            ...
            {
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new SecurityTokenException(SR.Format(SR.ID4290, claim)));
            }
        }
        ...
        /// <returns>Instance of <see cref="CoreWCF.IdentityModel.Claims.Claim"/></returns>
        private SysClaim DeserializeSysClaim(XmlDictionaryReader reader)
        {
            ...
            {
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new SecurityTokenException(SR.Format(SR.ID4289, reader.LocalName, reader.NamespaceURI)));
            }
        }
        ...
        private static string ReadRightAttribute(XmlDictionaryReader reader, SessionDictionary dictionary)
        {
            ...
            return string.IsNullOrEmpty(right) ? CoreWCF.IdentityModel.Claims.Rights.PossessProperty : right;
        }

        private static void WriteRightAttribute(SysClaim claim, SessionDictionary dictionary, XmlDictionaryWriter writer)
        {
            if (CoreWCF.IdentityModel.Claims.Rights.PossessProperty.Equals(claim.Right))
                return;
