...
namespace CoreWCF.IdentityModel.Tokens
{
    ...
    public abstract class UserNameSecurityTokenHandler : SecurityTokenHandler
    {
        ...
        /// token.
        /// </summary>
        /// <param name=nameof(reader)>XmlReader pointing to SecurityToken.</param>
        ...
        public override SecurityToken ReadToken(XmlReader reader)
        {
            ...
            while (reader.IsStartElement())
            {
                ...
                else if (reader.IsStartElement(WSSecurity10Constants.Elements.Password, WSSecurity10Constants.Namespace))
                {
                    ...
                    {
                        throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new NotSupportedException(SR.Format(SR.ID4059, type, WSSecurity10Constants.UPTokenPasswordTextValue)));
                    }
                ...
                {
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new XmlException(SR.Format(SR.ID4060, reader.LocalName, reader.NamespaceURI, WSSecurity10Constants.Elements.UsernameToken, WSSecurity10Constants.Namespace)));
                }
            }
            ...
            {
                throw new InvalidOperationException(SR.Format(SR.ID4061));
            }
        ...
        /// </summary>
        /// <param name=nameof(writer)>XmlWriter to write the token to.</param>
        /// <param name=nameof(token)>SecurityToken to be written.</param>
        ...
        public override void WriteToken(XmlWriter writer, SecurityToken token)
        {
            ...
            if (!(token is UserNameSecurityToken usernameSecurityToken))
            {
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperArgument(nameof(token), SR.Format(SR.ID0018, typeof(UserNameSecurityToken)));
            }
