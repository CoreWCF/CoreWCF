...
using CoreWCF.Xml.Serialization;
...
namespace CoreWCF.Description
{
    ...
    {
        protected readonly WsdlContractConversionContext _contractContext;
        protected readonly WsdlExporter _exporter;
        protected readonly OperationDescription _operation;
        protected readonly IOperationBehavior _extension;
        ...
        protected MessageExportContext ExportedMessages => GetMessageExportContext(_exporter);

        private void AddElementToSchema(XmlSchemaElement element, string elementNs, XmlSchemaSet schemaSet)
        {
            OperationDescription parentOperation = _operation;
        ...
        internal void ExportMessageContract()
        {
            ...
            OperationFormatter.Validate(_operation, IsRpcStyle(), IsEncoded());
            ExportKnownTypes();

            for (int messageIndex = 0; messageIndex < _operation.Messages.Count; messageIndex++)
            {
                ExportMessage(messageIndex, state);
            }

            if (!_operation.IsOneWay)
            {
                ExportFaults(state);
            }
            ...
            {
                EnsureXsdImport(schema.TargetNamespace, _contractContext.WsdlPortType.ServiceDescription);
            }
        }
        ...
        private void ExportMessage(int messageIndex, object state)
        {
            try
            {
                MessageDescription description = _operation.Messages[messageIndex];
                ...
                if (CreateMessage(description, messageIndex, out wsdlMessage))
                {
                    ...
                    StreamFormatter streamFormatter = StreamFormatter.Create(description, _operation.Name, isRequest);
        ...
        {
            foreach (FaultDescription fault in _operation.Faults)
            {
                ExportFault(fault);
            }
        }
        ...
        {
            return _operation.DeclaringContract != _contractContext.Contract;
        }

        private void ExportAnyMessage(WsdlNS.Message message, MessagePartDescription part)
        {
            XmlSchemaSet schemas = _exporter.GeneratedXmlSchemas;
        ...
        private void ExportFault(FaultDescription fault)
        {
            ...
            _contractContext.WsdlPortType.ServiceDescription.Messages.Add(faultMessage);
            AddMessagePart(faultMessage, "detail", elementName, null);

            // create a wsdl:fault to put inside the wsdl:portType/wsdl:operation
            WsdlNS.OperationFault operationFault = _contractContext.GetOperationFault(fault);
            WsdlExporter.WSAddressingHelper.AddActionAttribute(fault.Action, operationFault, _exporter.PolicyVersion);
        ...
        private XmlQualifiedName ExportFaultElement(FaultDescription fault)
        {
            ...
            XmlQualifiedName typeName = ExportType(fault.DetailType, fault.Name, _operation.Name, out xsdType);
            ...
            if (XmlName.IsNullOrEmpty(fault.ElementName))
            {
                ...
                if (elementName == null)
                {
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(SR.Format(SR.SFxFaultTypeAnonymous, _operation.Name, fault.DetailType.FullName)));
                }
            }
            else
            {
            ...
            ExportGlobalElement(elementName.Name, elementName.Namespace, true/*isNillable*/, typeName, xsdType, _exporter.GeneratedXmlSchemas);
            return elementName;
        }
        ...
        protected XmlSchemaSet SchemaSet => _exporter.GeneratedXmlSchemas;

        protected static WsdlNS.MessagePart AddMessagePart(WsdlNS.Message message, string partName, XmlQualifiedName elementName, XmlQualifiedName typeName)
        {
        ...
        private static void EnsureXsdImport(string ns, WsdlNS.ServiceDescription wsdl)
        {
        ...
        private static XmlSchema GetContainedSchema(WsdlNS.ServiceDescription wsdl, string ns)
        {
        ...
        protected bool CreateMessage(MessageDescription message, int messageIndex, out WsdlNS.Message wsdlMessage)
        {
            ...
            if (ExportedMessages.WsdlMessages.ContainsKey(new MessageDescriptionDictionaryKey(_contractContext.Contract, message)))
            {
            ...
            {
                typedMessageKey = new TypedMessageKey(message.MessageType, _operation.DeclaringContract.Namespace, GetExtensionData());
                if (ExportedMessages.TypedMessages.TryGetValue(typedMessageKey, out wsdlMessage))
                {
                    isNewMessage = false;
                }
            }
            else if (_operation.OperationMethod != null)
            {
                messageKey = new OperationMessageKey(_operation, messageIndex);
            ...
            WsdlNS.ServiceDescription wsdl = _contractContext.WsdlPortType.ServiceDescription;
            if (isNewMessage)
            {
            ...
            ExportedMessages.WsdlMessages.Add(new MessageDescriptionDictionaryKey(_contractContext.Contract, message), wsdlMessage);
        ...
        protected bool CreateHeaderMessage(MessageDescription message, out WsdlNS.Message wsdlMessage)
        {
            ...
            if (ExportedMessages.WsdlHeaderMessages.ContainsKey(new MessageDescriptionDictionaryKey(_contractContext.Contract, message)))
            {
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new ArgumentException(SR.MultipleCallsToExportContractWithSameContract));
            }
            ...
            if (message.IsTypedMessage)
            {
                ...
                {
                    ExportedMessages.WsdlHeaderMessages.Add(new MessageDescriptionDictionaryKey(_contractContext.Contract, message), wsdlMessage);
                    return false;
                }
            }
            ...
            _contractContext.WsdlPortType.ServiceDescription.Messages.Add(wsdlMessage);
            if (message.IsTypedMessage)
            {
        ...
        private string GetMessageName(MessageDescription messageDescription)
        {
            ...
            {
                string portTypeName = _contractContext.WsdlPortType.Name;
                string operationName = _contractContext.GetOperation(_operation).Name;

                string callbackString = _operation.IsServerInitiated() ? "Callback" : string.Empty;
                ...
                {
                    messageNameBase = string.Format(CultureInfo.InvariantCulture,
        ...
        {
            WsdlNS.Message wsdlBodyMessage = ExportedMessages.WsdlMessages[new MessageDescriptionDictionaryKey(_contractContext.Contract, messageDescription)];

            string messageNameBase = string.Format(CultureInfo.InvariantCulture,
        ...
        protected string GetFaultMessageName(string faultName)
        {
            string portTypeName = _contractContext.WsdlPortType.Name;
            string operationName = _contractContext.GetOperation(_operation).Name;
            string faultNameBase = string.Format(CultureInfo.InvariantCulture, "{0}_{1}_{2}_FaultMessage", portTypeName, operationName, faultName);
        ...
        private string GetUniqueMessageName(WsdlNS.ServiceDescription wsdl, string messageNameBase)
        {
        ...
        protected void AddParameterOrder(MessageDescription message)
        {
            ...
            WsdlNS.Operation wsdlOperation = _contractContext.GetOperation(_operation);
        ...
        {
            int count = -1;
            foreach (MessageDescription message in _operation.Messages)
            {
                foreach (MessagePartDescription part in message.Body.Parts)
                {
                    ParameterInfo paramInfo = part.AdditionalAttributesProvider as ParameterInfo;
        ...
        protected virtual void Compile()
        {
            ...
            SchemaHelper.Compile(SchemaSet, _exporter.Errors);
        }

        private class MessageBindingExporter
        {
            private readonly WsdlEndpointConversionContext _endpointContext;
            private readonly MessageExportContext _exportedMessages;
            private readonly EnvelopeVersion _soapVersion;
            private readonly WsdlExporter _exporter;
            ...
            internal MessageBindingExporter(WsdlExporter exporter, WsdlEndpointConversionContext endpointContext)
            {
                ...
                _exportedMessages = (MessageExportContext)exporter.State[typeof(MessageExportContext)];
                _soapVersion = SoapHelper.GetSoapVersion(endpointContext.WsdlBinding);
                _exporter = exporter;
            }

            internal void ExportMessageBinding(OperationDescription operation, Type messageContractExporterType)
            {
                WsdlNS.OperationBinding wsdlOperationBinding = _endpointContext.GetOperationBinding(operation);
                ...
                WsdlNS.SoapOperationBinding soapOperationBinding = SoapHelper.GetOrCreateSoapOperationBinding(_endpointContext, operation, _exporter);
                ...
                if (isRpc)
                {
                    WsdlNS.SoapBinding soapBinding = (WsdlNS.SoapBinding)_endpointContext.WsdlBinding.Extensions.Find(typeof(WsdlNS.SoapBinding));
                ...
                {
                    WsdlNS.MessageBinding wsdlMessageBinding = _endpointContext.GetMessageBinding(message);
                    WsdlNS.Message headerMessage;
                    if (_exportedMessages.WsdlHeaderMessages.TryGetValue(new MessageDescriptionDictionaryKey(_endpointContext.Endpoint.Contract, message), out headerMessage))
                    {
            ...
            private void ExportFaultBinding(FaultDescription fault, bool isEncoded, WsdlNS.OperationBinding operationBinding)
            {
                SoapHelper.CreateSoapFaultBinding(fault.Name, _endpointContext, _endpointContext.GetFaultBinding(fault), isEncoded);
            }

            private void ExportMessageBodyBinding(MessageDescription messageDescription, bool isRpc, bool isEncoded, WsdlNS.MessageBinding messageBinding)
            {
                WsdlNS.SoapBodyBinding bodyBinding = SoapHelper.GetOrCreateSoapBodyBinding(_endpointContext, messageBinding, _exporter);
                ...
                {
                    string ns;
                    if (!ExportedMessages.WrapperNamespaces.TryGetValue(new MessageDescriptionDictionaryKey(_endpointContext.ContractConversionContext.Contract, messageDescription), out ns))
                    {
                ...
                if (isEncoded)
                {
                    bodyBinding.Encoding = XmlSerializerOperationFormatter.GetEncoding(_soapVersion);
                }
            }

            private void ExportMessageHeaderBinding(MessageHeaderDescription header, XmlQualifiedName messageName, bool isEncoded, WsdlNS.MessageBinding messageBinding)
            {
#if DEBUG
                Fx.Assert(NamingHelper.IsValidNCName(messageName.Name), "Name value has to be a valid NCName.");
#endif
                WsdlNS.SoapHeaderBinding headerBinding = SoapHelper.CreateSoapHeaderBinding(_endpointContext, messageBinding);
                ...
                if (isEncoded)
                {
                    headerBinding.Encoding = XmlSerializerOperationFormatter.GetEncoding(_soapVersion);
                }
            }
            ...
            private MessageExportContext ExportedMessages => GetMessageExportContext(_exporter);
        }

        protected class MessageExportContext
        {
            internal readonly Dictionary<MessageDescriptionDictionaryKey, WsdlNS.Message> WsdlMessages = new Dictionary<MessageDescriptionDictionaryKey, System.Web.Services.Description.Message>();
            internal readonly Dictionary<MessageDescriptionDictionaryKey, WsdlNS.Message> WsdlHeaderMessages = new Dictionary<MessageDescriptionDictionaryKey, System.Web.Services.Description.Message>();
            internal readonly Dictionary<MessageDescriptionDictionaryKey, string> WrapperNamespaces = new Dictionary<MessageDescriptionDictionaryKey, string>();
            internal readonly Dictionary<TypedMessageKey, WsdlNS.Message> TypedMessages = new Dictionary<TypedMessageKey, WsdlNS.Message>();
            internal readonly Dictionary<TypedMessageKey, WsdlNS.Message> TypedHeaderMessages = new Dictionary<TypedMessageKey, WsdlNS.Message>();
            internal readonly Dictionary<OperationMessageKey, WsdlNS.Message> ParameterMessages = new Dictionary<OperationMessageKey, WsdlNS.Message>();
            internal readonly Dictionary<XmlQualifiedName, OperationElement> ElementTypes = new Dictionary<XmlQualifiedName, OperationElement>();
        }
        ...
        protected sealed class MessageDescriptionDictionaryKey
        {
            ...
            public override bool Equals(object obj)
            {
                ...
                if (key != null && key.MessageDescription == MessageDescription && key.Contract == Contract)
                {
                    return true;
                }
            ...
            {
                return Contract.GetHashCode() ^ MessageDescription.GetHashCode();
            }
        }
        ...
        internal sealed class TypedMessageKey
        {
            ...
            public override bool Equals(object obj)
            {
                ...
                if (key != null && key.type == type &&
                    key.contractNS == contractNS &&
                    key.extensionData.Equals(extensionData))
                {
                    return true;
                }
        ...
        internal sealed class OperationMessageKey
        {
            ...
            private readonly ContractDescription _declaringContract;

            public OperationMessageKey(OperationDescription operation, int messageIndex)
            {
                _methodInfo = operation.OperationMethod;
                _messageIndex = messageIndex;
                _declaringContract = operation.DeclaringContract;
            }

            public override bool Equals(object obj)
            {
                OperationMessageKey key = obj as OperationMessageKey;
                if (key != null && key._methodInfo == _methodInfo &&
                    key._messageIndex == _messageIndex &&
                    key._declaringContract.Name == _declaringContract.Name &&
                    key._declaringContract.Namespace == _declaringContract.Namespace)
                {
                    return true;
                }
            ...
            public override int GetHashCode()
            {
                return _methodInfo.GetHashCode() ^ _messageIndex;
            }
        }
        ...
        internal sealed class OperationElement
        {
            ...
            internal XmlSchemaElement Element => element;
            internal OperationDescription Operation => operation;
        }
    }
    ...
    internal class DataContractSerializerMessageContractExporter : MessageContractExporter
    {
        ...
        {
            return ((DataContractSerializerOperationBehavior)_extension).DataContractFormatAttribute.Style == OperationFormatStyle.Rpc;
        }
            ...
            if (description.Headers.Count > 0)
            {
                ...
                if (CreateHeaderMessage(description, out wsdlMessage))
                {
                    foreach (MessageHeaderDescription header in description.Headers)
                    {
                        ...
                        XmlQualifiedName typeName = ExportType(dataContractType, header.Name, _operation.Name, out xsdType);
        ...
        {
            MessageDescription description = _operation.Messages[messageIndex];
            WsdlNS.Message wsdlMessage = ExportedMessages.WsdlMessages[new MessageDescriptionDictionaryKey(_contractContext.Contract, description)];

            DataContractFormatAttribute dataContractFormatAttribute = ((DataContractSerializerOperationBehavior)_extension).DataContractFormatAttribute;
            XmlSchemaSequence wrapperSequence = null;
            ...
            if (OperationFormatter.IsValidReturnValue(description.Body.ReturnValue))
            {
                ...
                XmlQualifiedName typeName = ExportType(dataContractType, description.Body.ReturnValue.Name, _operation.Name, out xsdType);
            ...
            foreach (MessagePartDescription bodyPart in description.Body.Parts)
            {
                ...
                XmlQualifiedName typeName = ExportType(dataContractType, bodyPart.Name, _operation.Name, out xsdType);
        ...
        {
            foreach (Type knownType in _operation.KnownTypes)
            {
        ...
        {
            return new ExtensionData(((DataContractSerializerOperationBehavior)_extension).DataContractFormatAttribute);
        }

        private class ExtensionData
        {
            private readonly DataContractFormatAttribute _dcFormatAttr;
            internal ExtensionData(DataContractFormatAttribute dcFormatAttr)
            {
                _dcFormatAttr = dcFormatAttr;
            }

            public override bool Equals(object obj)
            {
                if (ReferenceEquals(_dcFormatAttr, obj))
                {
                    return true;
                }
                ...
                return _dcFormatAttr.Style == otherExtensionData._dcFormatAttr.Style;
            }
    ...
    internal class XmlSerializerMessageContractExporter : MessageContractExporter
    {
        ...
        protected override bool IsRpcStyle()
        {
            return ((XmlSerializerOperationBehavior)_extension).XmlSerializerFormatAttribute.Style == OperationFormatStyle.Rpc;
        }

        protected override bool IsEncoded()
        {
            return ((XmlSerializerOperationBehavior)_extension).XmlSerializerFormatAttribute.IsEncoded;
        }

        protected override object OnExportMessageContract()
        {
            object result = Reflector.ReflectOperation(_operation);
            if (result == null)
            {
                ...
                XmlSerializerOperationBehavior serializerBehavior = _extension as XmlSerializerOperationBehavior;
                if (serializerBehavior != null)
                {
                    result = Reflector.ReflectOperation(_operation, serializerBehavior.XmlSerializerFormatAttribute);
                }
            }
        ...
        {
            MessageDescription description = _operation.Messages[messageIndex];
            MessageDescriptionDictionaryKey key = new MessageDescriptionDictionaryKey(_contractContext.Contract, description);
            WsdlNS.Message wsdlMessage = ExportedMessages.WsdlMessages[key];
            ...
            if (membersMapping != null)
            {
                ...
                {
                    AddParameterOrder(_operation.Messages[messageIndex]);
                    ExportedMessages.WrapperNamespaces.Add(key, membersMapping.Namespace);
                }
            }
        }
        ...
        protected override object GetExtensionData()
        {
            return new ExtensionData(((XmlSerializerOperationBehavior)_extension).XmlSerializerFormatAttribute);
        }

        private class ExtensionData
        {
            private readonly XmlSerializerFormatAttribute xsFormatAttr;
            ...
            public override bool Equals(object obj)
            {
                if (ReferenceEquals(xsFormatAttr, obj))
                {
                    return true;
                }
            ...
            public override int GetHashCode() => 1; //This is never called
        }

        private void ExportMembersMapping(XmlMembersMapping membersMapping, WsdlNS.Message message, bool skipSchemaExport, bool isEncoded, bool isRpc, bool isDocWrapped, bool isHeader)
        {
            if (!skipSchemaExport)
            {
                if (isEncoded)
        ...
        private XmlSerializerOperationBehavior.Reflector Reflector
        {
            ...
            {
                object reflector;
                if (!_exporter.State.TryGetValue(typeof(XmlSerializerOperationBehavior.Reflector), out reflector))
                {
                    reflector = new XmlSerializerOperationBehavior.Reflector(_contractContext.Contract.Namespace, _contractContext.Contract.ContractType);
                    _exporter.State.Add(typeof(XmlSerializerOperationBehavior.Reflector), reflector);
                }
        ...
        private SoapSchemaExporter SoapExporter
        {
            ...
            {
                object soapExporter;
                if (!_exporter.State.TryGetValue(typeof(SoapSchemaExporter), out soapExporter))
                {
                    soapExporter = new SoapSchemaExporter(Schemas);
                    _exporter.State.Add(typeof(SoapSchemaExporter), soapExporter);
                }
        ...
        private XmlSchemaExporter XmlExporter
        {
            ...
            {
                object xmlExporter;
                if (!_exporter.State.TryGetValue(typeof(XmlSchemaExporter), out xmlExporter))
                {
                    xmlExporter = new XmlSchemaExporter(Schemas);
                    _exporter.State.Add(typeof(XmlSchemaExporter), xmlExporter);
                }
        ...
        private XmlSchemas Schemas
        {
            ...
            {
                object schemas;
                if (!_exporter.State.TryGetValue(typeof(XmlSchemas), out schemas))
                {
                    ...
                    _exporter.State.Add(typeof(XmlSchemas), schemas);
                }
        ...
        {
            XmlSchemas schemas = Schemas;
            XmlSchemaSet schemaSet = SchemaSet;
            if (schemas != null)
            {
                schemas.Compile(
                     (object sender, ValidationEventArgs args) =>
                     {
                         SchemaHelper.HandleSchemaValidationError(sender, args, _exporter.Errors);
                     },
                     false/*fullCompile*/
                );
