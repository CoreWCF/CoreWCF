...
namespace CoreWCF.Channels
{
    ...
    : this(binding, SafeCreateBindingElements(binding))
        ...
        public bool SupportsAuthorizationData
        {
            get
            {
                AuthorizationCapabilitiesBindingElement authorizationCapabilitiesBindingElement = Elements.Find<AuthorizationCapabilitiesBindingElement>();
                return authorizationCapabilitiesBindingElement != null;
            }
            set
            {
                AuthorizationCapabilitiesBindingElement authorizationCapabilitiesBindingElement = Elements.Find<AuthorizationCapabilitiesBindingElement>();
                if (authorizationCapabilitiesBindingElement != null)
                {
                    if (!value)
                    {
                        Elements.Remove(authorizationCapabilitiesBindingElement);
                    }
                }
                else
                {
                    if (value)
                    {
                        Elements.Insert(0, new AuthorizationCapabilitiesBindingElement());
                    }
                }
            }
        }
        ...
        private class AuthorizationCapabilitiesBindingElement : BindingElement
        {
            public override BindingElement Clone()
            {
                return new AuthorizationCapabilitiesBindingElement();
            }
